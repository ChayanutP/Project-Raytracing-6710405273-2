...
#เพิ่ม triangle object
class Triangle(Object):
    def __init__(self, v0, v1, v2, mat) -> None:
        super().__init__()
        self.v0 = v0
        self.v1 = v1
        self.v2 = v2
    ...
    def intersect(self, rRay, cInterval):
        ...
        h = rtu.Vec3.cross_product(rRay.getDirection(), edge2)
        a = rtu.Vec3.dot_product(edge1, h)
        ...
        q = rtu.Vec3.cross_product(s, edge1)
        v = f * rtu.Vec3.dot_product(rRay.getDirection(), q)

        if v < 0.0 or u + v > 1.0:
            return None

        t = f * rtu.Vec3.dot_product(edge2, q)

        if not cInterval.contains(t):
            return None

        hit_point = rRay.at(t)
        outward_normal = rtu.Vec3.unit_vector(rtu.Vec3.cross_product(edge1, edge2))

        hinfo = rtu.Hitinfo(hit_point, outward_normal, t, self.material)
        hinfo.set_face_normal(rRay, outward_normal)

        # UV mapping (barycentric)
        hinfo.set_uv(u, v)
